<?php namespace Bluechip\EdPixlr\Service;
   ...
   public function handlePixlrPost()
   {
	  try {
		 ...
		 if (in_array($state, array('new', 'copy', 'replace'))) {
			...
			$this->addMessage($this->translate('file_added_description'), $this->translate('file_added_title'), FlashMessage::OK);
		 }
	  } catch (\Exception $e) {
	  ...
	  if (!empty($ajaxCall)) {
		 $script = '<script>';
		 $script .= 'if (parent) { parent.window.location = parent.TYPO3.jQuery(\'a.go-back-to-list:first\').attr(\'href\'); }';
		 $script .= '</script>';
...
//		exit;
   ...
   protected function addMessage($description, $title = null, $severity = FlashMessage::INFO)
   {
	  $message = GeneralUtility::makeInstance(
			  'TYPO3\\CMS\\Core\\Messaging\\FlashMessage', $description, $title, $severity, true
	  );
