var pixlr = function() {
    ...
    function buildUrl(opt) {
        var url = '//apps.pixlr.com/'+ opt.service +'/?s=c';
        for(var attribute in opt) {
            if(attribute !== 'service' || attribute !== 'container') url += "&"+ attribute +"="+ escape(opt[attribute]);
        }
        return url;
    }
    var bo = {
        ...
        overlay: {
            show: function(options) {
                ...
                if (options.container) {
                    idiv = document.getElementById(options.container);
                } else {
